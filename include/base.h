...
namespace unet
{
    ...
    protected:
        ...
        std::function<int(const void *, size_t)> send_m = std::bind(&net_baseIPV6::send_tcp, this, std::placeholders::_1, std::placeholders::_2);
        std::function<int(void *, size_t)> recv_m = std::bind(&net_baseIPV6::recv_tcp, this, std::placeholders::_1, std::placeholders::_2);
        std::function<int()> close_m = std::bind(&net_baseIPV6::close_tcp, this);
        net_baseIPV6() noexcept;
        ~net_baseIPV6();
    ...
    class net_coreIPV6 : public net_baseIPV6
    {
    private:
    public:
        net_coreIPV6(int socket, const addrinfo cli, sock_type type_ = TCP_c, SSL *ssl_ = nullptr) noexcept;
        ~net_coreIPV6();

        addrinfo remote() const noexcept;
    };
